...
void ProcessReceiveEvent(ENetPacket* packet)
{
	...
	switch (recvBuf->type)
	{
		...
		case SG_NEWCLIENT:
		{
			...
			octr->sendwarpclock[octr->DriverID] = false; // this sendwarpclock is useless, i will remove it later
			alreadysended = false;
...
void StatePC_Game_EndRace()
{
	...
	if (boolAlreadySent_EndRace) {
		...
		if (prev_finishtimer > 6 || activePlayers == 1) {
			alreadysended = true;
		}

		//1 player race // all players finished
		if (octr->numDriversEnded == activePlayers && alreadysended && prev_finishtimer > 6 ) {
